...
if os.environ.get('RWKV_V7_ON') == '1':
    ...
    class RWKV_x070(MyModule):
        def __init__(self, model, strategy, version:str="x070"):
            global DTYPE, DEVICE
            super().__init__()
            self.eval()
        ...
        def forward(self, idx, state, full_output=False):
            ...
            if type(idx) is list:
                if len(idx) > 1:
                    return self.forward_seq(idx, state, full_output, self.version)
                else:
                    return self.forward_one(idx[0], state, self.version)
            else:
                return self.forward_one(idx, state, self.version)

        @MyFunction
        def forward_one(self, idx:int, state:List[torch.Tensor], version:str="x070"):
            with torch.no_grad(): 
                z = self.z
                x = z['emb.weight'][idx]
                ...
                for i in range(self.n_layer):
                    ...
                    if version == "x070":
                        ...
                        xx, state[i*3+0], state[i*3+1], v_first = RWKV_x078_TMix_one(i, self.n_head, self.head_size, xx, state[i*3+0], v_first, state[i*3+1],
                            z[att+'x_r'], z[att+'x_w'], z[att+'x_k'], z[att+'x_v'], z[att+'x_a'], z[att+'x_g'],
                            z[att+'w0'], z[att+'w1'], z[att+'w2'], z[att+'a0'], z[att+'a1'], z[att+'a2'], z[att+'v0'], z[att+'v1'], z[att+'v2'],
                            z[att+'g1'], z[att+'g2'], z[att+'k_k'], z[att+'k_a'], z[att+'r_k'],
                            z[att+'receptance1.weight'].t(), 
                            z[att+'receptance2.weight'].t(),
                            z[att+'key1.weight'].t(),
                            z[att+'key2.weight'].t(), 
                            z[att+'value1.weight'].t(),
                            z[att+'value2.weight'].t(), 
                            z[att+'output.weight'],
                            z[att+'ln_x.weight'], z[att+'ln_x.bias'])
                    x = x + xx
        ...
        @MyFunction
        def forward_seq(self, idx:List[int], state:List[torch.Tensor], full_output:bool=False, version:str="x070"):
            with torch.no_grad(): 
                z = self.z
                x = z['emb.weight'][idx]
                ...
                for i in range(self.n_layer):
                    ...
                    if version == "x070":
                        ...
                        xx, state[i*3+0], state[i*3+1], v_first = RWKV_x078_TMix_seq(i, self.n_head, self.head_size, xx, state[i*3+0], v_first, state[i*3+1],
                            z[att+'x_r'], z[att+'x_w'], z[att+'x_k'], z[att+'x_v'], z[att+'x_a'], z[att+'x_g'],
                            z[att+'w0'], z[att+'w1'], z[att+'w2'], z[att+'a0'], z[att+'a1'], z[att+'a2'], z[att+'v0'], z[att+'v1'], z[att+'v2'],
                            z[att+'g1'], z[att+'g2'], z[att+'k_k'], z[att+'k_a'], z[att+'r_k'],
                            z[att+'receptance1.weight'].t(), 
    ...
    def RWKV_x078_TMix_one(layer_id: int, H:int, N:int, x, x_prev, v_first, state, x_r, x_w, x_k, x_v, x_a, x_g, w0, w1, w2, a0, a1, a2, v0, v1, v2, g1, g2, k_k, k_a, r_k, 
                           R1_, R2_, K1_, K2_, V1_, V2_, 
                           O_, ln_w, ln_b):
        xx = x_prev - x
        xr, xw, xk, xv, xa, xg = x+xx*x_r, x+xx*x_w, x+xx*x_k, x+xx*x_v, x+xx*x_a, x+xx*x_g
    ...
    if os.environ.get('RWKV_CUDA_ON') == '1':
        ...
        @MyStatic
        def RWKV_x078_TMix_seq(layer_id: int, H:int, N:int, x, x_prev, v_first, state, x_r, x_w, x_k, x_v, x_a, x_g, w0, w1, w2, a0, a1, a2, v0, v1, v2, g1, g2, k_k, k_a, r_k, 
                               R1_, R2_, K1_, K2_, V1_, V2_,
                               O_, ln_w, ln_b):
            T = x.shape[0]
            xx = torch.cat((x_prev.unsqueeze(0), x[:-1,:])) - x
