...
if 'x070' in os.environ["RWKV_MY_TESTING"] or 'x078' in os.environ["RWKV_MY_TESTING"]:
    ...
    load(name="wind_backstepping", sources=[f'cuda/wkv7_cuda.cu', 'cuda/wkv7_op.cpp'], is_python_module=False, verbose=True,
...
# decomposed (left, right), has relu in between, has diag
class RWKV_Tmix_x059(MyModule):
    def __init__(self, args, layer_id):
        super().__init__()
        self.args = args
        self.layer_id = layer_id
...
# decomposed (left, right), no relu, no diag
class RWKV_Tmix_x058(MyModule):
    def __init__(self, args, layer_id):
        super().__init__()
        self.args = args
        self.layer_id = layer_id
...
# WC: continual training
class RWKV_Tmix_x078(MyModule):
    def __init__(self, args, layer_id):
        super().__init__()
        self.args = args
        self.layer_id = layer_id
        ...
        with torch.no_grad():
            ...
            self.time_shift = nn.ZeroPad2d((0, 0, 1, -1))
            # SVD for r,k,v,g
            self.receptance1 = nn.Linear(C, C // args.svdfac, bias=False)
            self.receptance2 = nn.Linear(C // args.svdfac, C, bias=False)

            self.key1 = nn.Linear(C, C // args.svdfac, bias=False)
            self.key2 = nn.Linear(C // args.svdfac, C, bias=False)

            self.value1 = nn.Linear(C, C // args.svdfac, bias=False)
            self.value2 = nn.Linear(C // args.svdfac, C, bias=False)
...
# decomposed weights, has relu, has diag
class RWKV_CMix_x059_r(MyModule):
...
# decomposed weights, no relu, no diag
class RWKV_CMix_x058_r(MyModule):
    # SVD + finetune: decomposed='r'
    # pretrain: decomposed='rkv'
    # (cf comments below
    def __init__(self, args, layer_id):
...
class RWKV_CMix_x059_rkv(MyModule):
    # SVD + finetune: decomposed='r'
    # pretrain: decomposed='rkv'
    # (cf comments below
...
class RWKV_CMix_x0594_rkv(MyModule):
    # pretrain: decomposed='rkv'
    # (cf comments below
    def __init__(self, args, layer_id, decomposed='r'):
        super().__init__()
...
class RWKV_CMix_x0596_rkv(MyModule):
    # pretrain: decomposed='rkv'
    # (cf comments below
    def __init__(self, args, layer_id, decomposed='r'):
        super().__init__()
...
class Block(nn.Module):
    def __init__(self, args, layer_id):
        ...
        #     args.NoReLu=True
        #     args.NoDiag=True
        # elif 'x059' == os.environ["RWKV_MY_TESTING"]:
        #     args.NoReLu=False
        #     args.NoDiag=False
        ...
        if self.layer_id == 0 and self.args.pre_ffn > 0:        # xzl: replace layer0 attn with ffn.. trick
            ...
            elif 'x078' in os.environ["RWKV_MY_TESTING"]:
                self.att = RWKV_Tmix_x078(args, layer_id)
